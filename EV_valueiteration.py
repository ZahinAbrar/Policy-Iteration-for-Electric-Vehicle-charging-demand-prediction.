...
def valueIteration(nowTime):
    ...
    for battIndex in range(0, MaxBattery + 1):
        ...
        # print(reward)
        Vfunction[battIndex][MaxTime] = round(reward, 4)
    doneError = 0.01
    while True:
...
def applyBellman(Vfunction, timeIndex, battIndex, dayTime):
    ...
    for changeBatt in generatePossibleCharges(battIndex, MaxBattery,timeIndex):
...
def generatePossibleCharges(battIndex, MaxBattery,timeIndex):
    ...
    return list(range(int(Demand[timeIndex]),MaxDeltaBatt+1))
...
def computeCostList(buySell, dayTime):
    ...
    for timeIndex in range(0, MaxTime):
        ...
        # print(value)
        price=math.exp((Demand[timeIndex]-low)/(high-low))*0.07
        income=(Demand[timeIndex]*price/EnergySample)*-1
        # print(income)
        # print(price)
        zoneList.append(Zone[timeIndex])
        cost = value + cost+income
...
def optimizer(sampletime=15,batterypanalty=0.01,numberOfhours=1*24,gamma=1,maxsolar=40,maxload=30,batterymaxenergy=20,energysample=10):
    ...
    # for i in range(0,96):
    #     print(Vtable[159][i],Vtable[199][i])
